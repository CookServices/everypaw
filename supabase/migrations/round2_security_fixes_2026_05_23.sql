-- Round 2 security fixes — 2026-05-23
-- C2: Atomic book credit consumption to prevent race conditions
...
BEGIN
  ...
  -- print plan users order freely, no credit needed
  IF v_plan = 'print' THEN
    RETURN true;
  END IF;
...
-- Restore a credit on Gelato failure (rollback)
...
BEGIN
  ...
  -- Only restore for non-print users (print users never consumed one)
  IF v_plan IS DISTINCT FROM 'print' THEN
