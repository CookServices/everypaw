...
export function buildConfirmSignupEmail(lang: "fr" | "en", confirmUrl: string): { subject: string; html: string } {
  if (lang === "fr") {
    return {
      ...
      html: baseLayout(`
        ...
        <p style="font-size:.9rem;color:${BRAND.muted};line-height:1.65;margin:0 0 24px;">Vous êtes à un clic de commencer à capturer les plus beaux moments de votre animal.</p>
  ...
  return {
    ...
    html: baseLayout(`
      ...
      <p style="font-size:.9rem;color:${BRAND.muted};line-height:1.65;margin:0 0 24px;">You're one click away from starting to capture your pet's most beautiful moments.</p>
  ...
  if (lang === "fr") {
    return {
      ...
      html: baseLayout(`
        ...
        <p style="font-size:.9rem;color:${BRAND.muted};line-height:1.65;margin:0 0 24px;">Vous avez demandé à réinitialiser votre mot de passe. Cliquez ci-dessous pour en choisir un nouveau :</p>
        ${colorSection("Votre lien d'accès expire dans 1 heure", "#A67C52", "#FDFAF5")}
        ${ctaButton(resetUrl, "Réinitialiser mon mot de passe")}
        <p style="font-size:.78rem;color:#9A8070;margin:16px 0 0;line-height:1.5;">Si vous n'avez pas demandé cette réinitialisation, ignorez cet email, votre mot de passe reste inchangé.</p>
      `, "", "fr"),
    };
  }
  return {
    ...
    html: baseLayout(`
      ...
      <p style="font-size:.9rem;color:${BRAND.muted};line-height:1.65;margin:0 0 24px;">You requested to reset your password. Click below to choose a new one:</p>
      ${colorSection("Your reset link expires in 1 hour", "#A67C52", "#FDFAF5")}
      ${ctaButton(resetUrl, "Reset my password")}
      <p style="font-size:.78rem;color:#9A8070;margin:16px 0 0;line-height:1.5;">If you didn't request this reset, please ignore this email, your password remains unchanged.</p>
    `, "", "en"),
  };
}
  ...
  if (lang === "fr") {
    return {
      ...
      html: baseLayout(`
        ...
        <p style="font-size:.9rem;color:${BRAND.muted};line-height:1.65;margin:0 0 24px;">Nous n'avons pas pu traiter le paiement de votre abonnement Everypaw.</p>
        ...
        <p style="font-size:.78rem;color:#9A8070;margin:16px 0 0;line-height:1.5;">Stripe effectuera plusieurs tentatives automatiquement. Si le paiement échoue, votre accès Premium sera suspendu.</p>
      `, "", "fr"),
    };
  }
  return {
    ...
    html: baseLayout(`
      ...
      <p style="font-size:.9rem;color:${BRAND.muted};line-height:1.65;margin:0 0 24px;">We were unable to process the payment for your Everypaw subscription.</p>
