...
> Archive complète, du plus récent au plus ancien. CLAUDE.md reprend les 2 dernières entrées pour
> les avoir sous la main au démarrage : elles figurent donc aux deux endroits, c'est voulu.

---

### ✅ Vérification — second balayage `public-pages-purge` confirmé en réel (2026-09-19)

**Le second balayage de PP-5 (Session 75) venait d'une trouvaille, jamais observé contre de vraies orphelines depuis sa mise en prod.** Trois photos `pet-photos/public/` créées le 2026-09-17 (07:46, 07:54, 08:26 UTC), envoyées puis abandonnées avant soumission du formulaire — `public_pages` à 0 ligne, donc rien ne les référençait. Elles avaient moins de 24 h au passage du cron du 18 à 04:00 UTC (épargnées, comme prévu), environ 44 h à celui du 19. Contrôle après ce second passage : les trois ont disparu, `pet-photos/public/` à 0 objet, `public_pages` toujours à 0 ligne. Cas attendu confirmé, rien à corriger.

---

### ✅ Session 77 — Item #21 clos, les hommages cessent d'arriver en codes d'entité (2026-09-18)

**`POST /api/memorial/tributes` stocke désormais `author_name` et `message` bruts, simplement détourés des espaces.** Les cinq relectures ont été auditées une par une avant de retirer l'échappement, puisque c'est lui qui aurait protégé un rendu qui n'échapperait pas : trois passent par JSX (page mémorial, page publique, onglet de modération), `preview-pdf` applique `escapeHtml` sur son gabarit HTML et `book-pdf` rend du `<Text>` react-pdf. Aucune ne touche `dangerouslySetInnerHTML`. **Le livre imprimé était touché aussi**, c'est la trouvaille de la passe : `preview-pdf` échappait par-dessus l'échappement stocké, donc un hommage payé et relié serait sorti en `&amp;#x27;`. Vérifié en bout de chaîne avec `Jean d'Arc` et `On n'oubliera pas <ce chien> & ses "betises".` — restitué tel quel dans l'onglet de modération et sur la page mémorial publique, `&lt;ce chien&gt;` dans la source HTML donc aucune injection, et `&#x27;` une seule fois dans le gabarit du PDF. **Aucun backfill n'a été écrit : la table `memorial_tributes` comptait zéro ligne**, vérifié avant et après. **Les sujets d'emails ont été repris dans la foulée**, le défaut n'étant pas isolé : `birthday-check`, `daily-prompts`, `on-this-day`, `streak-alert` et la notification d'hommage mettaient tous un nom d'animal échappé dans le sujet, et `contact` envoyait « Commande &amp; livraison ». Les corps gardent leur variable échappée, les sujets prennent la valeur brute. Les quatre autres emails porteurs d'un nom (`weekly-reminder`, `monthly-story`, `first-story-nudge`, `retention-emails`, `pet-members`) faisaient déjà la distinction et n'ont pas bougé. Le sujet de `contact` a été vérifié en production par un envoi réel, objet « Commande & livraison » : `[Contact] Commande & livraison` est arrivé dans la boîte, sans `&amp;`. Les cinq autres restent vérifiés par relecture et par le type-checker, pas par envoi, ce sont des crons qui ne tournent ni en local ni en preview.
...
## 🗄️ Déplacé de CLAUDE.md le 2026-09-02 (backlog #20)

CLAUDE.md dépassait sa propre limite de 700 lignes. Le contenu ci-dessous en a été retiré tel quel :
il documente des incidents résolus et des décisions déjà appliquées, plus des règles opérationnelles.
Les règles qui restent utiles au quotidien ont été reformulées, en plus court, dans CLAUDE.md.

---

### Deployment & Staging — version longue (état au 2026-09-01)
...
### Test du webhook Stripe en local via Stripe CLI (2026-07-13)
...
### Checklist de mise en production — items faits
...
### Backlog dette technique, items clos (état au 2026-09-01)

> Repris tel quel de CLAUDE.md. Les items encore ouverts (#4, #8, #12(b)) sont restés là-bas.
...
- **#21 Hommages stockés échappés en HTML (Session 77, PR [#166](https://github.com/CookServices/everypaw/pull/166) puis [#167](https://github.com/CookServices/everypaw/pull/167))** — cause : `POST /api/memorial/tributes` appliquait `escapeHtml` à l'écriture sur `author_name` et `message`, et chaque relecture échappe au rendu, donc l'apostrophe du message d'un proche arrivait en `&#x27;`. Fix : stockage brut, simplement détouré des espaces. Les cinq relectures ont été auditées avant de retirer l'échappement, puisque c'est lui qui aurait couvert un rendu qui n'échappe pas — trois passent par JSX (page mémorial, page publique, onglet de modération), `preview-pdf` applique `escapeHtml` sur son gabarit HTML, `book-pdf` rend du `<Text>` react-pdf, aucune ne touche `dangerouslySetInnerHTML`, et l'email de notification ne contient pas le texte de l'hommage. **Le livre imprimé était touché aussi** : `preview-pdf` échappait par-dessus la valeur stockée, un hommage payé et relié serait sorti en `&amp;#x27;`. Vérification : hommage déposé par la vraie route avec `Jean d'Arc` et `On n'oubliera pas <ce chien> & ses "betises".`, restitué tel quel dans l'onglet de modération et sur la page mémorial publique, `&lt;ce chien&gt;` dans la source HTML donc aucune injection, `&#x27;` une seule fois dans le gabarit du PDF. **Aucun backfill** : la table comptait zéro ligne, vérifié avant et après. PR #167 a étendu le même défaut aux sujets d'emails, où `escapeHtml` n'a rien à faire — `birthday-check`, `daily-prompts`, `on-this-day`, `streak-alert`, la notification d'hommage et `contact` (« Commande &amp; livraison » à chaque message reçu) ; les corps gardent leur variable échappée, les sujets prennent la valeur brute, et les cinq envois qui faisaient déjà la distinction n'ont pas bougé. Le sujet de `contact` a été vérifié en production par un envoi réel (`[Contact] Commande & livraison`, sans `&amp;`) ; les cinq autres restent vérifiés par relecture et par le type-checker, pas par envoi, ce sont des crons qui ne tournent ni en local ni en preview.
...
- **#19 Chaînes i18n mortes citant d'anciens prix (Session 67)** : `order.product_price` (`29,00 €` / `$29.00`) n'est lue par aucun composant, le prix affiché vient de `calcGelatoBookPrice`. Vérifié au `grep` : `landing.free_book`, `landing.pricing_book_note` et `landing.order_book` (tous à `29 €`, plus `49 €` et `9,99 €` ailleurs dans le fichier) ne sont lues nulle part non plus — ce sont des restes de l'ancien catalogue (livre à la carte, print mensuel) supprimé le 2026-07-07. Rien n'est affiché à l'utilisateur aujourd'hui, donc c'est du nettoyage, pas un bug. Antérieur à la Session 67, repéré en traitant #17. À faire : passer les clés `messages/{en,fr}.json` au crible des usages réels et supprimer les orphelines, dans les deux fichiers à la fois.

- **#20 CLAUDE.md dépasse sa propre limite (Session 67)** : le fichier fait 866 lignes alors que la consigne en tête dit de rester sous 700. Le dépassement est antérieur et s'aggrave à chaque session. Fix = pousser vers `docs/SESSIONS.md` ce qui n'est plus opérationnel, en priorité les entrées de backlog barrées (#6, #9, #11, #13, #14, #15, #16, #17, #18 pèsent une cinquantaine de lignes à elles seules) plutôt que les conventions, qui sont la partie réellement lue à chaque session.
