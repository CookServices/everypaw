...
Échapper à l'écriture ne protège rien qu'un rendu correct ne protège déjà, et ne se voit pas tant qu'on ne relit pas la valeur stockée. **Corollaire : un sujet d'email n'est pas du HTML.** Les corps d'emails s'échappent, les sujets jamais — garder deux variables quand les deux partent dans le même envoi (`petName` échappé pour le corps, `pet.name` brut pour le sujet).
